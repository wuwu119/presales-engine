...
- `opportunities/{slug}/analysis/rfp.yaml`
- `opportunities/{slug}/analysis/analysis.md`
- `${PRESALES_HOME}/knowledge/company-profile.yaml`
- `${PRESALES_HOME}/knowledge/products/*.yaml`
- 模板（用户优先）：
  - `${PRESALES_HOME}/templates/outline-{language}.yaml` — 章节大纲
  - `${PRESALES_HOME}/templates/chapters/*.md` — 章节正文片段（可无）
  - 回退：`${CLAUDE_PLUGIN_ROOT}/templates/...`

## 输出

- `opportunities/{slug}/draft/outline.md` — 章节大纲（含评分项映射）
- `opportunities/{slug}/draft/chapters/{NN-name}.md` — 每章一个文件

## 行为流程

### Phase 1: 生成大纲（`--outline-only` 模式止步于此）

1. 读取 `templates/outline-{language}.yaml`（默认跟随 `rfp.yaml.meta.language`）
...
- **禁止虚假案例** — 引用的客户案例必须在 `company-profile.case_references` 或 `knowledge/cases/` 中存在
