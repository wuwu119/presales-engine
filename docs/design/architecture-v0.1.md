...
1. **程序和数据严格分离** — 插件只读（`${CLAUDE_PLUGIN_ROOT}`），数据可写（`${PRESALES_HOME}` 默认 `~/售前/`）。卸载插件不影响用户数据。
...
| `PRESALES_HOME` | 用户数据根目录（可写） | `~/售前/` | 三层 resolution，见下 |
...
3. 默认 `~/售前/` — 无前导点，可见于 Finder/`ls`，便于用户手工编辑
...
```
${PRESALES_HOME}/                    # 默认 ~/售前/；交互式 setup 时用户可指定父目录
...
**`cases/` vs `知识库/客户案例/`**：前者是已归档的 opportunity 目录树（原始 RFP + 生成的标书 + 复盘），后者是可复用的客户案例资料库（客户授权可公开引用的材料）。两者概念不同，命名必须区分。
...
| 字段 | 内容 |
|------|------|
| 输入 | `{slug}/招标文件/原件/*.{pdf,docx,md,txt}` |
| 输出 | `{slug}/分析/rfp.yaml` + `{slug}/招标文件/extracted.md` + `{slug}/meta.yaml` |
...
| 字段 | 内容 |
|------|------|
| 输入 | `{slug}/分析/rfp.yaml` + `知识库/company-profile.yaml` + `知识库/产品档案/*.yaml` |
| 输出 | `{slug}/分析/analysis.md` |
...
| 字段 | 内容 |
|------|------|
| 输入 | `rfp.yaml` + `analysis.md` + `outline.md`（可选） + `模板/` |
| 输出 | `{slug}/草稿/outline.md` + `{slug}/草稿/章节/*.md` |
