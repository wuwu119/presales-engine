...
"""scan: diff 知识库/资质证书/ against company-profile.yaml, list unregistered files.
apply: append approved payload entries into company-profile.yaml.qualifications[].

v0.2 MVP scope: only --type certs. Other knowledge types extend this script in v0.3.
"""
...
def cmd_scan(args: argparse.Namespace) -> int:
    if args.type not in SUPPORTED_TYPES:
        _die(EXIT_PAYLOAD_INVALID, f"v0.2 仅支持 --type certs，收到 {args.type}")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_scan.add_argument("--type", default="certs", help="知识类型（v0.2 仅 certs）")
    p_scan.set_defaults(func=cmd_scan)

    p_apply = sub.add_parser("apply", help="应用批准 payload 写入 company-profile.yaml")
